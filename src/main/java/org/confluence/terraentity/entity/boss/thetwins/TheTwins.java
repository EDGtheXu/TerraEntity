...
public class TheTwins extends AbstractTerraBossBase implements Boss {
    ...
    private float mhp1 = 0, mhp2 = 0;

    public record SkillParams(
            Spazmatism.SkillParams spazmatismParams,
            Retinazer.SkillParams retinazerParams
    ) {
    ...
    public @NotNull AABB getBoundingBoxForCulling() {
        ...
        return new AABB(spazmatism.position(), retinazer.position());
    }
    ...
    public void tick() {
        ...
        if (level() instanceof ServerLevel level) {
            if (tickCount == 50) { // 暂时使用延迟同步id的方法，但如果区块卸载后再加载会出问题
                if (spazmatismUUID == null || retinazerUUID == null) {
                    discard();
                } else {
                    bindEyes(level);
                }
            } else if (tickCount > 50 && (tickCount & 31) == 0) { // 更新位置
                bindEyes(level); // 防止初次绑定失败
                if (spazmatism != null && spazmatism.isAlive() && retinazer != null && retinazer.isAlive()) {
                    ...
                    if (spazmatism != null && spazmatism.isAlive()) {
                        setPos(spazmatism.position().add(0, 5, 0));
                    } else if (retinazer != null && retinazer.isAlive()) {
                        setPos(retinazer.position().add(0, 5, 0));
                    } else {
                        discard();
                    }
                }
    ...
    private void bindEyes(ServerLevel serverLevel) {
        if (retinazer == null && retinazerUUID != null && serverLevel.getEntity(retinazerUUID) instanceof Retinazer re && re.isAlive()) {
            this.retinazer = re;
            entityData.set(DATA_RETINAZER_ID, retinazer.getId());
            re.ownerUUID = getUUID();
        }
        if (spazmatism == null && spazmatismUUID != null && serverLevel.getEntity(spazmatismUUID) instanceof Spazmatism sp && sp.isAlive()) {
            this.spazmatism = sp;
    ...
    public void readAdditionalSaveData(@NotNull CompoundTag tag) {
        ...
        if (tag.contains("spazmatismUUID")) {
            this.spazmatismUUID = tag.getUUID("spazmatismUUID");
        }
        if (tag.contains("retinazerUUID")) {
            this.retinazerUUID = tag.getUUID("retinazerUUID");
        }
    }
