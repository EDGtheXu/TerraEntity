...
//                this.laserTick = Mth.clamp(this.laserTick + (this.getFlag(this.laserFlag)? 1 : -1), 0, this.laserPreTick);
                laser.update(this.getFlag(this.laserFlag)? 1 : -1);
                this.tickParts();
...
//        return this.laserLength * Mth.clamp((this.laserTick + partialTick * (this.getFlag(this.laserFlag)? 1 : -1)) / this.laserPreTick, 0, 1);
    }
...
//            living.addDeltaMovement(new Vec3(0,2,0));
    ...
    private float getHeadYOffset() {
        if (false) {
            ...
            return (float)(adouble1[2] - adouble[2]);
        }
    }
...
//    }
    ...
    protected static class DragonMovement {
...
//        float movementSpeed = baseSpeed * (alignmentFactor * proximityFactor + (1.0F - proximityFactor));
        ...
        private void move(MoverType moverType, Vec3 deltaMovement) {
            this.mob.move(moverType, deltaMovement);
        }

        private void setDeltaMovement(Vec3 multiply) {
        ...
        private void moveRelative(float movementSpeed, Vec3 vec3) {
